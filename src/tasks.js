...
const tasks = {
  ...
  init: function(name) {
    ...
    mkdir(name);
    cp('-R', path.resolve(__dirname, '..', 'templates', 'app/*'), path.resolve(CWD, name));
  ...
  'init-redux': function(name) {
    ...
    mkdir(name);
    cp('-R', path.resolve(__dirname, '..', 'templates', 'redux/*'), path.resolve(CWD, name));
    console.log('Project', name, 'create success');
  },

  'init-prerender': function() {
    cp('-R', path.resolve(__dirname, '..', 'templates', 'prerender/*'), path.resolve(CWD));
  },
  ...
  build: function() {
    ...
    exec(webpack + ' --config ' + path.resolve(__dirname, '..', 'webpack', 'webpack.base.config.js') + ' --progress');
  },
  ...
  'build-server': function() {
    ...
    exec(webpack + ' --config ' + path.resolve(__dirname, '..', 'webpack', 'webpack.server.config.js') + ' --progress');
  },
  ...
  component: function(name) {
    ...
    console.log('目录:' + [breezeCfg.base, breezeCfg.components, name].join('/'));
  },
  ...
  page: function(name) {
    ...
    console.log('目录:' + [breezeCfg.base, breezeCfg.pages.name, name].join('/'));
  },
